...
{
    private static Logger logger_ = Logger.getLogger(VersionVectorClock.class);
    public static VersionVectorClock MIN_VALUE = new VersionVectorClock(ArrayUtils.EMPTY_BYTE_ARRAY);
    ...
    public ClockRelationship compare(IClock other)
    {
        ...
        ContextRelationship rel = contextManager.compare(context, ((VersionVectorClock)other).context());
        switch (rel)
        {
            case EQUAL:
...
{
    public void serialize(IClock vc, DataOutput out) throws IOException
    {
        FBUtilities.writeByteArray(((VersionVectorClock)vc).context(), out);
    }
